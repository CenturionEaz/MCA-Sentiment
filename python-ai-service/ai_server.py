...
║    • WordCloud — frequency-based keyword visualization      ║
...
from config import (
    ...
    WORDCLOUD_WIDTH, WORDCLOUD_HEIGHT, WORDCLOUD_MAX_WORDS,
    WORDCLOUD_COLORMAP, WORDCLOUD_BG_COLOR, DOMAIN_STOPWORDS,
    API_PORT, API_HOST, MAX_INPUT_TEXTS,
)
from preprocessing import clean_for_sentiment, clean_for_wordcloud, clean_text
...
# ═════════════════════════════════════════════════════════════
# ENDPOINT: /wordcloud — Enhanced Word Cloud
# ═════════════════════════════════════════════════════════════

@app.route("/wordcloud", methods=["POST"])
def wordcloud():
    """
    Word cloud with stopword removal and domain filtering.

    Improvements:
      - NLTK English stopwords removed
      - Domain-specific governance/policy stopwords removed
      - Better text cleaning pipeline
      - Higher quality output

    Request:  { "texts": ["comment1", ...] }
    Response: { "success": true, "data": { "path": "static/wordcloud.png" } }
    """
    start = time.time()
    ...
    try:
        # Clean with aggressive stopword removal
        cleaned = clean_for_wordcloud(texts, domain_stopwords=DOMAIN_STOPWORDS)
        combined = " ".join(cleaned)

        if not combined.strip():
            return api_response(error="No meaningful words after cleaning", status=400)
        ...
        output_path = os.path.join(STATIC_DIR, "wordcloud.png")
        wc.to_file(output_path)

        elapsed_ms = round((time.time() - start) * 1000, 1)
        logger.info(f"/wordcloud: {len(texts)} texts → {elapsed_ms}ms")

        return api_response(data={"path": "static/wordcloud.png"}, meta={
            "processing_time_ms": elapsed_ms,
            "unique_words": len(wc.words_),
        })

    except Exception as e:
        logger.error(f"Word cloud error: {e}", exc_info=True)
        return api_response(error=f"Word cloud generation failed: {str(e)}", status=500)
